...
final class RootArraySimilarComparator extends Comparator
{
    public function accepts($expected, $actual): bool
    {
        return is_array($expected)
            && is_array($actual)
            && (all(instance_of(Root::class), $expected)
                && all(instance_of(Root::class), $actual));
    }
    ...
    {
        $exists = fn(Root $expected) => any(
            fn(Root $actual) => TestUtils::isSimilar($expected, $actual),
            $actualArray
        );
